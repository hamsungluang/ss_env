...
const { get_location, saveCodeToFile } = require("./tools/func_tools");
...
// --------------加载rs6-----------------------------------
function rs6() {
    let urlString = "https://pmos.ah.sgcc.com.cn:20080/#/outNet";
    const workScripts = [
        "./work/rs6.js",
        "./work/rs6_ts.js",
    ]
    let frameworkCode = ''
    let workScriptCode = 'document.cookie;'
    if (!CONFIG.DEBUG) {
...
function get_a_bogus() {
    let frameworkCode = ""
    let urlString = "https://www.douyin.com/video/7487819295116823808";
    ...
    const envCodeLoader = new EnvCodeLoader(frameworkCode, workScripts, workScriptCode);
    const code = envCodeLoader.load();
    // saveCodeToFile(code, "./get_data_demo/douyin/token_env_code.js")
    const vm = new VMRunner(urlString);
    const result = vm.run(code);
    console.log(result)
};
